          ...
          <button
            onClick={() => {
              setVisibleComments((prev) => {
                return prev.length >= commentsData.length ? commentsData.slice(0, 2) : commentsData.slice(0, prev.length + 2);
              });
            }}
            className="text-blue-600"
          >
            {visibleComments.length >= commentsData.length ? "Show Less" : "Show More"}
          </button>
        )}
      </div>

      <div className="flex flex-col items-center justify-center">
        <p className="text-lg mb-2">{(commentsData.length === 0) ? 'Be the first one to rate' : 'How do you rate this place?'}</p>
        <Rating
          count={5}
